...
/**
 * Integration coverage for the storage cleanup worker against an in-process S3-compatible server
 * (s3mock): dropping a managed table or volume queues a cleanup task, and the worker deletes the
 * object prefix while leaving siblings untouched. The worker's {@code FileOperations} is stubbed to
 * hand its {@link S3FileIO} the s3mock client directly, because credential vending does not yet
 * expose a storage-endpoint override; everything else -- the repositories, the DB task, and the
 * worker's claim/delete/finish cycle -- is real.
 */
@ExtendWith(S3MockExtension.class)
class S3StorageCleanupWorkerE2ETest {
