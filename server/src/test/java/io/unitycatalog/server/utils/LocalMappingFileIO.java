 ...
 *
 * <p>It also implements {@link SupportsPrefixOperations} (like {@link SimpleLocalFileIO}),
 * rewriting a caller's cloud prefix to the local dir, so a test's storage cleanup ({@code
 * listPrefix} / {@code deletePrefix}) runs against the mapped local files.
 */
public class LocalMappingFileIO implements SupportsPrefixOperations {
  ...
  public Iterable<FileInfo> listPrefix(String prefix) {
    return delegate.listPrefix(toLocalUri(prefix));
  }

  @Override
  public void deletePrefix(String prefix) {
    requireWritable("delete");
    delegate.deletePrefix(toLocalUri(prefix));
  }
