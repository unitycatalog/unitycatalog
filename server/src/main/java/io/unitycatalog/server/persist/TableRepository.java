...
public class TableRepository {
  ...
  /**
   * One page of the tables in a schema that carry UniForm Iceberg metadata, and the token for the
   * page after it. An empty {@code nextPageToken} means the listing is complete. The names are the
   * uniform tables only, so a page can be empty while more pages remain.
   */
  public record UniformIcebergTablePage(List<String> tableNames, Optional<String> nextPageToken) {}

  /**
   * Lists the tables in a schema that carry UniForm Iceberg metadata, one repository page at a
   * time.
   *
   * <p>Whether a table is uniform is read from the very row the page was read from, so a table
   ...
   */
  public UniformIcebergTablePage listUniformIcebergTables(
        ...
        session -> {
          ...
          List<String> tableNames =
              page.stream()
                  ...
                  .toList();
          return new UniformIcebergTablePage(
