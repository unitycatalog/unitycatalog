        ...
        .forEachRemaining(
            field -> {
              ...
              columnWidths.put(
                  key,
                  Math.min(
                      Math.max(columnWidths.getOrDefault(key, key.length()), valueLength),
                      MAX_COLUMN_WIDTH));
            });
  }
  ...
  private static void printHeaders(JsonNode node, Map<String, Integer> columnWidths) {
    ...
    node.fieldNames()
        .forEachRemaining(
            field ->
                System.out.printf(
                    " %-" + columnWidths.get(field) + "s |",
                    truncate(field, columnWidths.get(field))));
    System.out.println();
    System.out.print("|");
    node.fieldNames()
        .forEachRemaining(field -> System.out.print("-".repeat(columnWidths.get(field) + 2) + "|"));
    System.out.println();
  }
    ...
    node.fields()
        .forEachRemaining(
            field -> {
              ...
              System.out.printf(
                  " %-" + columnWidths.get(field.getKey()) + "s |",
                  truncate(value, columnWidths.get(field.getKey())));
            });
    System.out.println();
  }
