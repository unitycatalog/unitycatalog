...
| **allowed-maintenance-operations** | [**List**](DeltaMaintenanceOperation.md) | Maintenance operations that the catalog permits in addition to the operations that the Delta protocol permits by default. A missing or empty list permits no additional operations. A client must not perform an operation that it does not recognize.  | [optional] [default to null] |
