...
| **client-maintenance-operations** | [**List**](DeltaMaintenanceOperation.md) | Additional maintenance operations that the catalog permits for this table. A missing or empty list permits no additional operations. A client must not perform an operation that it does not recognize.  | [optional] [default to null] |
