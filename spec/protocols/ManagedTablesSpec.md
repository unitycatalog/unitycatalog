...
- **Maintenance policy**: table responses now identify maintenance operations that clients may perform in addition to the Delta protocol defaults.
...
- **Get the table**: After the table is created successfully, the client can identify the table by its catalog, schema, and name to retrieve the table entry.
  ...
  - The server responds with the table metadata (schema, properties including protocol-derived entries, etag, table identity), all possibly-unbackfilled ratified commits in `commits`, the `latest-table-version` tracked by Unity Catalog, and the table's additional client maintenance operations.
...
Field Name | Data Type | Description | Optional/Required
-|-|-|-
additional-client-maintenance-operations | array of string | Maintenance operations the client may perform in addition to the Delta protocol defaults. Unity Catalog returns `OPTIMIZE` and `VACUUM` for managed tables. Missing or empty means no additions; names are case-sensitive, and clients ignore duplicates and unknown names. | required
...
**Field requiredness.** The required markers in the table above describe the server's obligations for managed Delta tables. The shared `delta.yaml` response schema marks `additional-client-maintenance-operations`, `commits`, `latest-table-version`, and the `last-commit-*` fields optional because the same response shape also serves external Delta tables, which have no catalog-managed maintenance policy or commit log.
...
```json
{
  "additional-client-maintenance-operations": ["OPTIMIZE", "VACUUM"],
  "metadata": {
