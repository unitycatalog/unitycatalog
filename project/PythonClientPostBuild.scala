...
object PythonClientPostBuild {
  ...
  def processGeneratedFiles(
    ...
    patchDataTypeModule(log, baseDir, openApiOutputDir)
  }

  /**
   * Copies the hand-written data_type_module.py into the generated delta package
   * and appends an import to the generated __init__.py so the patch is applied
   * automatically when the package is imported.
   */
  def patchDataTypeModule(
      log: Logger,
      baseDir: File,
      openApiOutputDir: String
  ): Unit = {
    val srcModule = baseDir.toPath.resolve("src")
      .resolve("unitycatalog").resolve("delta").resolve("serde").resolve("data_type_module.py")
    val targetDir = Paths.get(openApiOutputDir, "src", "unitycatalog", "delta")
    val targetSerdeDir = targetDir.resolve("serde")
    val targetModule = targetSerdeDir.resolve("data_type_module.py")
    val targetInit = targetDir.resolve("__init__.py")

    if (!Files.exists(srcModule)) {
      log.warn(s"data_type_module.py not found at $srcModule, skipping patch")
      return
    }
    ...
    log.info(s"Copied data_type_module.py to $targetModule")
