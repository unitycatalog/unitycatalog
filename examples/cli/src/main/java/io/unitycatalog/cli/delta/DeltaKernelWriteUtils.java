...
/**
 * Helper class to write a sample Delta table.
 * Generates random data conforming to the given schema and writes it to the given table path.
 * For adding more data types, add the corresponding methods in the ColumnVectorUtil class.
 * Most of the code in this class has been copied from <a href="https://github.com/delta-io/delta/blob/master/kernel/examples/kernel-examples/src/main/java/io/delta/kernel/examples/BaseTableWriter.java">BaseTableWriter.java</a>
 */
public class DeltaKernelWriteUtils {
    ...
    public static String writeSampleDataToDeltaTable(String tablePath, List<ColumnInfo> columns,
        ...
        try {
            ...
            writeSampleDataToExistingDeltaTable(engine, DeltaKernelUtils.substituteSchemeForS3(tablePath), schema, createVsUpdate);
    ...
    public static void writeSampleDataToExistingDeltaTable(Engine engine, String tablePath, StructType tableSchema, boolean createVsUpdate) throws IOException {
     ...
     * @return
     */
    static FilteredColumnarBatch generateUnpartitionedDataBatch(StructType tableSchema, int count, int offset) {
    ...
    public static ColumnVector[] createVectorsFromSchema(StructType schema, int count , int offset) {
        ...
        for (int i = 0; i < schema.length(); i++) {
            ...
            if (dataType == IntegerType.INTEGER) {
                values = generateRandomValues(dataType, count, field.getName().equalsIgnoreCase("id") , offset);
    ...
    @SuppressWarnings("unchecked")
    public static <T> List<T> generateRandomValues(DataType dataType, int count, boolean isId, int offset) {
