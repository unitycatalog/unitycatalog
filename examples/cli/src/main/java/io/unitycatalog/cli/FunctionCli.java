...
public class FunctionCli {
    ...
    private static String createFunction(FunctionsApi functionsApi, JSONObject json) throws JsonProcessingException, ApiException {
        ...
        return objectWriter.writeValueAsString(functionsApi.createFunction(new CreateFunctionRequest()
                .functionInfo(createFunction)));
    }

    private static String listFunctions(FunctionsApi functionsApi, JSONObject json) throws JsonProcessingException, ApiException {
        ...
        return objectWriter.writeValueAsString(functionsApi.listFunctions(catalogName,
                schemaName, 100, null).getFunctions());
    }

    private static String executeFunction(FunctionsApi functionsApi, JSONObject json) throws ApiException {
        ...
        try {
            return PythonInvoker.invokePython(functionsApi.getFunction(functionFullName)
    ...
    private static String getFunction(FunctionsApi functionsApi, JSONObject json) throws JsonProcessingException, ApiException {
        String functionFullName = json.getString(CliParams.FULL_NAME.getServerParam());
        return objectWriter.writeValueAsString(functionsApi.getFunction(functionFullName));
    }

    private static String deleteFunction(FunctionsApi functionsApi, JSONObject json) throws ApiException {
        String functionFullName = json.getString(CliParams.FULL_NAME.getServerParam());
