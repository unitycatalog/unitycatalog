...
public class CliUtils {
    ...
    public static void printEntityHelp(String entity) {
        ...
        System.out.printf("Valid sub-commands for %s are: %s\n", entity, String.join(", ",entityOptions.keySet()));
        System.out.printf("For detailed help on %s sub-commands, use bin/uc %s <sub-command> --help\n", entity, entity);
    }
    ...
    public static void printHelp() {
        ...
        System.out.println();
        System.out.println("By default, the client will connect to UC running locally at http://localhost:8080\n");
        System.out.println("To connect to specific UC server, use --server https://<host>\n");
        System.out.println("Currently, auth using bearer token is supported. Please specify the token via --auth_token <PAT Token>\n");
        System.out.println("For detailed help on entity specific operations, use bin/uc <entity> --help");
    }
    ...
    public static Map<String, String> extractProperties(ObjectMapper objectMapper, JSONObject node) {
        ...
        if (node.has(CreateTable.JSON_PROPERTY_PROPERTIES)) {
            ...
            try {
                ...
                throw new IllegalArgumentException("Failed to deserialize properties field " + escapedJson, e);
            }
        }
        return null;
    }
     ...
     * e.g., id INT, name String, age INT
     * @param columnsString
     * @return
     */
    public static List<ColumnInfo> parseColumns(String columnsString) {
        ...
        while (matcher.find()) {
            try {
                ...
                columnInfo.setTypeJson(String
                        .format("{\"name\":\"%s\",\"type\":\"%s\",\"nullable\":%s,\"metadata\":{}}",
                                name,
                                typeNameVsJsonText.getOrDefault(typeName, typeName.toLowerCase(Locale.ROOT))
                                        + setPrecisionAndScale(columnInfo, typeName, params), "true"));
    ...
    public static FunctionParameterInfos parseInputParams(JSONObject json) {
        ...
        for (int i = 0; i < inputParams.length; i++) {
            ...
            FunctionParameterInfo param = new FunctionParameterInfo()
                    ...
                    .typeJson(String.format("{\"name\":\"%s\",\"type\":\"%s\",\"nullable\":%s,\"metadata\":{}}",name,
                            typeNameVsJsonText.getOrDefault(typeName, typeName.toLowerCase()), "true"))
                    .position(i);
