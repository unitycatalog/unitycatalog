...
public class VolumeCli {
    ...
    private static String createVolume(VolumesApi volumesApi, JSONObject json) throws JsonProcessingException, ApiException {
    ...
    private static String listVolumes(VolumesApi volumesApi, JSONObject json) throws JsonProcessingException, ApiException {
        ...
        return objectWriter.writeValueAsString(volumesApi.listVolumes(catalogName, schemaName,
                        100 , null).getVolumes());
    }

    private static String getVolume(VolumesApi volumesApi, JSONObject json) throws JsonProcessingException, ApiException {
    ...
    private static String writeRandomFileInVolume(VolumesApi volumesApi
            ,TemporaryVolumeCredentialsApi tempCredApi, JSONObject json) throws ApiException {
        String volumeFullName = json.getString(CliParams.FULL_NAME.getServerParam());
        ...
        Configuration conf = DeltaKernelUtils.getHDFSConfiguration(baseURI,
                tempCredApi.generateTemporaryVolumeCredentials(
                                new GenerateTemporaryVolumeCredential()
                                        .volumeId(volumeInfo.getVolumeId()).operation(VolumeOperation.WRITE_VOLUME))
                        .getAwsTempCredentials());
        FileSystem fs = DeltaKernelUtils.getFileSystem(URI.create(
                DeltaKernelUtils.substituteSchemeForS3(volumeLocation)), conf);
        try {
            org.apache.hadoop.fs.Path path = new org.apache.hadoop.fs.Path(
                    DeltaKernelUtils.substituteSchemeForS3(baseURI.toString()));
            ...
            org.apache.hadoop.fs.Path filePath = new org.apache.hadoop.fs.Path(DeltaKernelUtils.substituteSchemeForS3(volumeLocation), randomFileName);
    ...
    private static String readVolume(VolumesApi volumesApi,
                                     TemporaryVolumeCredentialsApi tempCredApi,
                                     JSONObject json) throws ApiException {
        String volumeFullName = json.getString(CliParams.FULL_NAME.getServerParam());
        VolumeInfo volumeInfo = volumesApi.getVolume(volumeFullName);
        String volumeLocation = volumeInfo.getStorageLocation();
        String relativePath = json.has(CliParams.PATH.getServerParam()) ?
        ...
        Configuration conf = DeltaKernelUtils.getHDFSConfiguration(relativeURI,
                tempCredApi.generateTemporaryVolumeCredentials(
                new GenerateTemporaryVolumeCredential()
                        .volumeId(volumeInfo.getVolumeId()).operation(VolumeOperation.READ_VOLUME))
                        .getAwsTempCredentials());
        URI relativeURIWithS3AScheme = URI.create(DeltaKernelUtils.substituteSchemeForS3(relativeURI.toString()));
        FileSystem fs = DeltaKernelUtils.getFileSystem(URI.create(
                DeltaKernelUtils.substituteSchemeForS3(volumeLocation)), conf);
        ...
        try {
            ...
            if (fileStatus.isDirectory()) {
                ...
                for (FileStatus status : fileStatuses) {
                    ...
                    fileList.add(relativeURIWithS3AScheme.relativize(entityPath) + " " + (status.isDirectory() ? "[directory]" : "[file]"));
                }
                return String.join("\n", fileList);
                ...
                throw new CliException("The specified path is neither a file nor a directory. " + fileStatus.getPath());
            }
        } catch (IOException e) {
    ...
    public static URI resolveURI(URI baseURI, String relativePath) {
        ...
        if (!resolvedPath.startsWith(basePath)) {
            throw new CliException("Resolved URI is outside the base URI path. Read Failed. " + resolvedPath);
        }

        return resolvedURI;
    }

    private static String updateVolume(VolumesApi apiClient, JSONObject json) throws JsonProcessingException, ApiException {
        String volumeFullName = json.getString(CliParams.FULL_NAME.getServerParam());
        ...
        return objectWriter.writeValueAsString(apiClient.updateVolume(volumeFullName, updateVolumeRequest));
    }
