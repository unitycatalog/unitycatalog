...
public class UnityCatalogCli {
    ...
    public static void main(String[] args) {
        ...
        Arrays.stream(CliParams.values()).
                forEach(cliParam -> options.addOption(
                        Option.builder().longOpt(cliParam.val()).hasArg().build()));
        options.addOption("h", "help", false, "Print help message.");

        // Add server specific options
        options.addOption(Option.builder().longOpt(CliUtils.SERVER).hasArg().desc("UC Server to connect to. Default is reference server.").build());
        options.addOption(Option.builder().longOpt(CliUtils.AUTH_TOKEN).hasArg().desc("PAT token to authorize uc requests").build());
        options.addOption(Option.builder().longOpt(CliUtils.OUTPUT).hasArg().desc("To indicate CLI output format preference").build());
    ...
    private static boolean validateCommand(CommandLine cmd) {
        ...
        if (subArgs.length < 1) {
            ...
            CliUtils.CliOptions allowedOptions = CliUtils.cliOptions.get(entity.toLowerCase())
