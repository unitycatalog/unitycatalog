...
public class TableCli {
    ...
    private static void handleTableStorageLocation(String storageLocation, List<ColumnInfo> columnInfos) {
        if (!(storageLocation.startsWith("s3://") || storageLocation.startsWith("file:/") || storageLocation.startsWith("/"))) {
            throw new CliException("Storage location must start with s3:// or file:/ or be an absolute local filesystem path.");
        }
    ...
    private static String listTables(TablesApi tablesApi, JSONObject json) throws JsonProcessingException, ApiException {
        return objectWriter.writeValueAsString(tablesApi.listTables(json.getString(CliParams.CATALOG_NAME.getServerParam()),
                json.getString(CliParams.SCHEMA_NAME.getServerParam()),
                100, null).getTables());
    }

    private static String getTable(TablesApi tablesApi, JSONObject json) throws JsonProcessingException, ApiException {
        String fullName = json.getString(CliParams.FULL_NAME.val());
        return objectWriter.writeValueAsString(tablesApi.getTable(fullName));
    }

    private static String readTable(TemporaryTableCredentialsApi temporaryTableCredentialsApi,
                                    TablesApi tablesApi, JSONObject json) throws ApiException {
        ...
        try {
            return DeltaKernelUtils.readDeltaTable(info.getStorageLocation(),
                    getTemporaryTableCredentials(temporaryTableCredentialsApi, tableId, TableOperation.READ), maxResults);
        } catch (Exception e) {
            throw new CliException("Failed to read delta table " + info.getStorageLocation(), e);
        }
    }

    private static String writeTable(TemporaryTableCredentialsApi temporaryTableCredentialsApi,
                                     TablesApi tablesApi, JSONObject json) throws ApiException {
        String fullTableName = json.getString(CliParams.FULL_NAME.getServerParam());
        TableInfo info = tablesApi.getTable(fullTableName);
        if (!DataSourceFormat.DELTA.equals(info.getDataSourceFormat())) {
        ...
        String tableId = info.getTableId();
        try {
            DeltaKernelWriteUtils.writeSampleDataToDeltaTable(info.getStorageLocation(), info.getColumns(),
                    getTemporaryTableCredentials(temporaryTableCredentialsApi, tableId, TableOperation.READ_WRITE));
        } catch (Exception e) {
            throw new CliException("Failed to write sample data to delta table " + info.getStorageLocation(), e);
        }
        return EMPTY;
    }
    ...
    public static AwsCredentials getTemporaryTableCredentials(TemporaryTableCredentialsApi apiClient,
                                                          String tableId,
                                                          TableOperation operation)
            throws ApiException {
        GenerateTemporaryTableCredentialResponse temporaryTableCredentialResponse = apiClient
                .generateTemporaryTableCredentials(new GenerateTemporaryTableCredential()
                        .tableId(tableId).operation(operation));
