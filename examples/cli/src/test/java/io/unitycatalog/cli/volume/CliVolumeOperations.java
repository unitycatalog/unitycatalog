...
public class CliVolumeOperations implements VolumeOperations {
    ...
    public VolumeInfo createVolume(CreateVolumeRequestContent createVolumeRequest) {
        List<String> argsList = new ArrayList<>(List.of(
                "volume", "create"
                , "--full_name", createVolumeRequest.getCatalogName() + "." + createVolumeRequest.getSchemaName() + "." +  createVolumeRequest.getName()
                , "--storage_location", createVolumeRequest.getStorageLocation()));
    ...
    @Override
    public VolumeInfo updateVolume(String volumeFullName, UpdateVolumeRequestContent updateVolumeRequest) {
        String[] args = addServerAndAuthParams(List.of(
                "volume", "update"
                , "--full_name", volumeFullName
                , "--new_name", updateVolumeRequest.getNewName()
                , "--comment", updateVolumeRequest.getComment())
                , config);
