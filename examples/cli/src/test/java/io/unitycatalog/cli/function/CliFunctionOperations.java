...
public class CliFunctionOperations implements FunctionOperations {
    ...
    public FunctionInfo createFunction(CreateFunctionRequest createFunctionRequest) {
        String fullName = createFunctionRequest.getFunctionInfo().getCatalogName() + "." +
                createFunctionRequest.getFunctionInfo().getSchemaName() + "." +
                createFunctionRequest.getFunctionInfo().getName();
        String inputParams =  createFunctionRequest.getFunctionInfo().getInputParams().getParameters()
                .stream().map(p -> p.getName() + " " + p.getTypeName()).collect(Collectors.joining(", "));
        List<String> argsList = new ArrayList<>(List.of(
                "function", "create"
                , "--full_name", fullName
                , "--input_params", inputParams
                , "--data_type", createFunctionRequest.getFunctionInfo().getDataType().name()));
    ...
    public FunctionInfo getFunction(String functionFullName) {
        String [] args = addServerAndAuthParams(List.of(
                "function", "get",
                "--full_name", functionFullName), config);
        JsonNode jsonNode = executeCLICommand(args);
        return objectMapper.convertValue(jsonNode, FunctionInfo.class);
    }
