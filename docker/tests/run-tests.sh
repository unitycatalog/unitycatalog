...
#
# Start container from a prebuilt image (Keycloak required for auth tests):
#   docker compose -f docker/oidc/compose.yaml -f docker/oidc/compose.docker-tests.yaml up -d --force-recreate keycloak
...
#   UC_ENABLE_OIDC=0           skip merging Keycloak OIDC settings (docker mode)
