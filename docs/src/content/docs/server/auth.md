...
First, let’s give our user account (`bobbie@rocinante`) `USE CATALOG`, `USE SCHEMA`, and `SELECT` permissions. Notice how we’re providing access using the three-part naming convention of _catalog, schema, and table_. As we provide each level of permissions, we progress down the three parts.
...
Within the `duckdb` session, let’s query the `unity.default.numbers` table. Note the `CREATE SECRET` statement uses the auth_token (`$token`) thus please fill the `TOKEN` field in with your saved `$token` value.
