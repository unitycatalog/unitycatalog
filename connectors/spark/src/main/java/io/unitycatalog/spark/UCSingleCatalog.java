...
public class UCSingleCatalog implements TableCatalog, SupportsNamespaces {
  ...
  static final ThreadLocal<Boolean> LOAD_DELTA_CATALOG =
      ThreadLocal.withInitial(() -> true);
  static final ThreadLocal<Boolean> DELTA_CATALOG_LOADED =
  ...
  public void initialize(String name, CaseInsensitiveStringMap options) {
    ...
    UCProxy proxy = new UCProxy(uri, tokenProvider, renewCredEnabled, apiClient, tablesApi,
        temporaryCredentialsApi);
    proxy.initialize(name, options);
    if (LOAD_DELTA_CATALOG.get()) {
      try {
        delegate = (TableCatalog) Class.forName("org.apache.spark.sql.delta.catalog.DeltaCatalog")
  ...
  public Table createTable(
    ...
    if (!hasExternalClause && !hasLocationClause && !isPathTable) {
      ...
      List<String> restrictedProps = Arrays.asList(
      ...
      Map<String, String> credentialProps = CredPropsUtil.createTableCredProps(
          renewCredEnabled,
      ...
      try {
        cred = temporaryCredentialsApi.generateTemporaryPathCredentials(
      ...
      Map<String, String> credentialProps = CredPropsUtil.createPathCredProps(
          renewCredEnabled,
  ...
  public Table createTable(Identifier ident, StructType schema,
    ...
    throw new UnsupportedOperationException(
        "deprecated `createTable` should not be called");
  }
  ...
  static void setCredentialProps(Map<String, String> props,
   ...
   *
   * This method creates a three-part name in the format `catalog.schema.table`
   * by combining the catalog name with the schema name (from the identifier's
   * namespace) and table name. It is NOT backtick quoted like what is usually
   * used in SQL statements even if the names have special characters like hyphens.
   *
   * Example:
   ...
   * catalogName=catalog, ident=((schema1, schema2), table): throws
  ...
  private static class UCProxy implements TableCatalog, SupportsNamespaces {
    ...
    public Table loadTable(Identifier ident) throws NoSuchTableException {
      ...
      TableIdentifier identifier = new TableIdentifier(
          t.getName(),
          Option.apply(t.getSchemaName()),
          Option.apply(t.getCatalogName()));
      ...
      TemporaryCredentials temporaryCredentials;
      try {
        temporaryCredentials = temporaryCredentialsApi
      ...
      Map<String, String> extraSerdeProps = CredPropsUtil.createTableCredProps(
          renewCredEnabled,
      ...
      List<String> partitionColumnNames = partitionCols.entrySet().stream()
      ...
      scala.collection.immutable.Map<String, String> storageProps =
          JavaConverters.mapAsScalaMap(storageProperties).toMap(
              scala.Predef$.MODULE$.<scala.Tuple2<String, String>>conforms());
      scala.collection.immutable.Map<String, String> emptyMap =
          JavaConverters.mapAsScalaMap(new HashMap<String, String>()).toMap(
              scala.Predef$.MODULE$.<scala.Tuple2<String, String>>conforms());
      ...
      CatalogTable sparkTable = new CatalogTable(
          identifier,
    ...
    public Table createTable(Identifier ident, StructType schema,
      ...
      String isManagedLocationStr =
          properties.get(TableCatalog.PROP_IS_MANAGED_LOCATION);
      boolean isManagedLocation = isManagedLocationStr != null
          && isManagedLocationStr.equalsIgnoreCase("true");
      ...
      if (isManagedLocation) {
        ...
        if (!format.equalsIgnoreCase(DataSourceFormat.DELTA.name())) {
          throw new RuntimeException(
              "Unity Catalog does not support non-Delta managed table.");
        }
      ...
      List<ColumnInfo> columns = IntStream.range(0, schema.fields().length)
          .mapToObj(i -> {
      ...
      // `createTable` already.
      Map<String, String> propertiesToServer = properties.entrySet().stream()
    ...
    public Map<String, String> loadNamespaceMetadata(String[] namespace)
      ...
      metadata.put(SchemaInfo.JSON_PROPERTY_CATALOG_NAME,
      ...
      metadata.put(SchemaInfo.JSON_PROPERTY_CREATED_AT,
          schema.getCreatedAt() != null
              ? schema.getCreatedAt().toString() : "null");
      metadata.put(SchemaInfo.JSON_PROPERTY_UPDATED_AT,
          schema.getUpdatedAt() != null
              ? schema.getUpdatedAt().toString() : "null");
