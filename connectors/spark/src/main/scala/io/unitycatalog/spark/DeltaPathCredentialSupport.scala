...
private[spark] object DeltaPathCredentialSupport {
        ...
        if (catalogTable.asInstanceOf[Option[_]].isDefined) {
          ...
          if (!isCloudPath(path)) {
            ...
            val tableOptions = table.getClass
              ...
              .invoke(table)
              .asInstanceOf[Map[String, String]]
            if (hasCredentialKeys(tableOptions)) {
              None
            } else {
              val creds = uc.vendPathCredentialConfWithFallback(spark, path)
              if (creds.isEmpty) {
  ...
  private def hasCredentialKeys(options: Map[String, String]): Boolean =
    options.keys.exists(k => k.startsWith("fs.") || k.startsWith("option.fs."))
