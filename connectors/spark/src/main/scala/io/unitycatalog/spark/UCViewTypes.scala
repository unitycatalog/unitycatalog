...
private[spark] object UCViewTypes {
   ...
   *
   *   - `Some(s)` means: appears in `listViews`, `loadRelation` / `loadView` builds a `View`
   *     labeled with `s`, and `createView` accepts `s` as the requested
  ...
  /**
   * Used by `toView` to label the built `View` with the right Spark
   ...
   * `dropView`, `loadRelation`, ...); the caller has a UC `TableType` (e.g. from `getUCTableLike(...)`).
